...
//            let updatedScreenPath = "\(Bundle.main.bundleIdentifier!)/\(screenPath)".lowercased()
			OptiLogger.debug("report screen event of \(screenTitle)")
			DispatchQueue.main.async {
				self.tracker?.track(view: [screenTitle], url: URL(string:"http://\(screenPath)"))

				let originalEvent = PageVisitEvent(customURL: SHA1.hexString(from: screenPath)!.replacingOccurrences(of: " ", with: ""), pageTitle: screenTitle, category: category)
