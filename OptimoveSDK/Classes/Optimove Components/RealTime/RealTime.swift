...
class RealTime: OptimoveComponent
{
	...
	func report(originalEvent: OptimoveEvent, withConfigs config: OptimoveEventConfig)
	{
		...
		realTimeQueue.async
		{
			...
			self.deviceStateMonitor.getStatus(of: .internet)
			{ (online) in
				guard online else {
				...
				do {
					...
					NetworkManager.post(toUrl: URL(string: Optimove.sharedInstance.realTime.metaData.realtimeGateway + "reportEvent")!, json: data)
					{ (response, error) in
