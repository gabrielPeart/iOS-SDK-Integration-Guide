...
```swift
Optimove.sharedInstance.reportScreen(screenTitle:screenPath:category:)
```

The `screenTitle` argument should represent the identifier of the current `scene` that the user is routing to, while the `screenPath` represent the full route to this `scene` inthe format of `path/to/screen` or via an array (`["path","to","screen"]`)<br>
To support more complex view hierarchies, you may also specify the scene `category` in the last argument.<br>
 ...
 override func viewDidAppear(_ animated: Bool)
    {
       ...
       Optimove.sharedInstance.reportScreenVisit(screenTitle: "sceneTitle", screenPath: "path/to/scene", category: "myCategory")
    }
```
>Note: The `reportScreen` method should only be called after the initialization success callback was received.
